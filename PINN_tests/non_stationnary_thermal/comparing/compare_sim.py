...
sim_res = np.load("comparing/res_BC.npy").mean(1)
...
ax.plot_wireframe(X[:, 0, :], Time[:, 0, :], sim_res, label="simulation result", color='b') 
plt.legend()
plt.show()


im = plt.imshow((Ts.mean(1) - sim_res).T, extent=[0., 10., 205., 0.], aspect='auto')
plt.colorbar(im, orientation='horizontal', label='\u0394T (°C)')
plt.xlabel('x (cm)')
plt.ylabel('t (s)')
plt.show()

im2 = plt.imshow(np.abs((Ts.mean(1) - sim_res).T / sim_res.T), extent=[0., 10., 205., 0.], aspect='auto', vmax=5., vmin=0.)
