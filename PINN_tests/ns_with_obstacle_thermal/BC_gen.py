...
def main_dist(plotting=True, training=True):
    ...
    if plotting:
        ...
        dist = as_block(vect, dist)
        true_dist = as_block(vect, final_dist)

        fig = plt.figure()
        ax = fig.add_subplot(projection='3d')
        ax.plot_wireframe(X[:, 0, :], Time[:, 0, :], dist.mean(1), label="NN", color='r')
        ax.plot_wireframe(X[:, 0, :], Time[:, 0, :], true_dist[:, 0, :], label="calculated distance", color='b')
        plt.legend()
        plt.show()
...
def main_BC(plotting=True, training=True):
    ...
    if plotting:
        ...
        BC_inter = as_block(vect, BC_inter)

        fig = plt.figure()
        ax = fig.add_subplot(projection='3d')
        ax.plot_wireframe(X[:, 0, :], Time[:, 0, :], np.nanmean(BC_inter, 1), label="NN")
        plt.legend()
        plt.show()
