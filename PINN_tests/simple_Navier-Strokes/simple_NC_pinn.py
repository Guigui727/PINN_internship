...
class PINN(tf.keras.Model):
    ...
    def train_step(self, data): #function called at each epoch
        ...
        with tf.GradientTape() as gradtape:
            ...
            inlet_pos = tf.where(vals[:, 0] > 0.)[:, 0]
            loss_inlet = tf.math.reduce_mean(tf.math.squared_difference(tf.gather(u, inlet_pos), tf.gather(vals[:, 0], inlet_pos))) + tf.math.reduce_mean(tf.math.squared_difference(tf.gather(v, inlet_pos), tf.gather(vals[:, 1], inlet_pos)))
            wall_pos = tf.where(vals[:, 0] == 0.)[:, 0]
            loss_wall = tf.math.reduce_mean(tf.math.squared_difference(tf.gather(u, wall_pos), tf.gather(vals[:, 0], wall_pos))) + tf.math.reduce_mean(tf.math.squared_difference(tf.gather(v, wall_pos), tf.gather(vals[:, 1], wall_pos)))
            outlet_pos = tf.where(vals[:, 2] == 0.)[:, 0]
            loss_outlet = tf.math.reduce_mean(tf.math.squared_difference(tf.gather(p, outlet_pos), tf.gather(vals[:, 2], outlet_pos)))
            loss = loss_u + loss_v + loss_cons + loss_inlet + loss_outlet + loss_wall #summing the mse
        ...
        return {"loss":loss, "loss_u":loss_u, "loss_v":loss_v, "loss_cons":loss_cons, "loss_wall":loss_wall, "loss_inlet":loss_inlet, "loss_outlet":loss_outlet}
...
hid = tf.keras.layers.Dense(20, activation="relu")(hid)
outp = tf.keras.layers.Dense(3, activation="linear")(hid)
...
model.fit(vect, vals, epochs=2000, batch_size=vect.shape[0])
